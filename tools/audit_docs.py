...
# ⚠ STALE 목록 관리: 값이 바뀌면 옛 값을 추가한다. 단 **옛 값이 나중에 다시 정상값이
#    될 수 있다** — "2.85%"는 BTC의 옛 20일 고저폭이라 넣었다가, 창이 굴러가며 60일
#    고저폭이 2.79% → 2.85%가 되어 정상값이 됐다(2026-09-21). 그래서 뺐다.
#    → 창이 굴러가며 값이 변하는 '측정치'는 STALE에 넣지 않고, 규칙·비교 표현만 넣는다.
STALE = ["-0.432", "+0.904", "+0.257", "±1.5%", "4.5배", "±0.57%",
